...
require_once (__DIR__.'/kannel.php');
require_once (__DIR__.'/edge.php');
require_once (__DIR__.'/beconn.php');
require_once(__DIR__.'/cdc.php');
require_once (__DIR__.'/conf/contenido.php');
require_once (__DIR__.'/conf/envios_mt.php');
require_once(__DIR__.'/conf/cdc_data.php');
...
if($conn)
{
      if(mysql_selectdb($namedb,$conn))
      {
        $sql = "SELECT clubs_SERVICE.id_club,clubs_SERVICE.name_club,clubs_SERVICE.id_carrier,clubs_SERVICE.route,".
          ...
          " WHERE clubs_CARRIER.id_carrier = $id_carrier and clubs_COBRO_AUTO.hour_session = '$hora' and clubs_SERVICE.`status` = 'enable' and clubs_COBRO_AUTO.last_exe < '$fecha' and clubs_COBRO_AUTO.$str_day_field='1' and clubs_COBRO_AUTO.status = 'enable'";
          ...
          while($row_servicio = mysql_fetch_array($response))
          {
              ...
              $envios->insertar_masivo($id_club,$puntos_bc,$my_id,$contenido_data['id_detalle_service']);
              $count = 0;
              while(($row_cliente = mysql_fetch_assoc($clientes)) && ($contenido_data))
