...
error_log(date("Ymd") . " Today: $today Hora: $hora Fecha: $fecha WD: $wd STR_DAY_FIELD: $str_day_field CARRIER: $id_carrier\n", 3, "/var/log/cobros_bc_club/cobros_bc.log");
...
if($conn)
{
      if(mysql_selectdb($namedb,$conn))
      {
        $sql = "SELECT clubs_SERVICE.id_club,clubs_SERVICE.name_club,clubs_SERVICE.id_carrier,clubs_SERVICE.route,".
          ...
          " WHERE clubs_CARRIER.id_carrier = $id_carrier and clubs_COBRO_AUTO.hour_session = '$hora' and clubs_SERVICE.`status` = 'enable' and clubs_COBRO_AUTO.last_exe < '$fecha' and clubs_COBRO_AUTO.$str_day_field='1' and clubs_COBRO_AUTO.status = 'enable' ORDER BY clubs_COBRO_AUTO.id_clubs DESC";
	echo $sql . "\n";
		...
		echo $sql . "\n";
              ...
              " as dias_alta FROM clubs_MSISDN WHERE clubs_MSISDN.id_clubs = $id_club  AND clubs_MSISDN.state = 'enable' AND clubs_MSISDN.msisdn NOT IN (SELECT clubs_ENVIOS_DIARIOS.msisdn FROM clubs_ENVIOS_DIARIOS WHERE clubs_ENVIOS_DIARIOS.id_clubs = $id_club AND clubs_ENVIOS_DIARIOS.eventos_cobrados = 1) ORDER BY clubs_MSISDN.points DESC";
		echo $sql . "\n";
              ...
              switch($row_servicio['type_send'])
              {
                ...
                case 'CDC':
                    ...
                    //error_log(date("Ymd") . "$response_free $response_paid $id_delivery\n", 3, "/var/log/cobros_bc_club/cobros_bc.log");
                    break;
              ...
              while(($row_cliente = mysql_fetch_array($clientes)) && ($contenido_data))
              {
                  ...
                  if ($row_servicio['type_send'] <> 'CDC') { ### SOLO GUARDAR LOS QUE NO SON CDC POR QUE NO ESTÁ SINCRONIZADA LA BASE DE DATOS, LOS COBROS SE TOMAN DE LAS NOTIFICACIONES ###
              ...
              } else if ($row_servicio['type_send'] == 'CDC')
              {
                  ...
                  echo "Envio a CDC";
                  //error_log(date("Ymd") . "$response_paid\n", 3, "/var/log/cobros_bc_club/cobros_bc.log");
