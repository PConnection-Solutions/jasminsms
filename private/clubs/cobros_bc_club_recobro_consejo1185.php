  ...
  echo $sql . "\n";
          ...
          while($row_servicio = mysql_fetch_array($response)) // While
          {
            ...
            if($id_club == 31){
              ...
              $sql = "update clubs_COBRO_AUTO set last_exe = DATE(CONVERT_TZ(NOW(), 'UTC', 'America/Tegucigalpa')) where id_broad_conf = $id_conf";
    echo $sql . "\n";
              mysql_query($sql,$conn);
              
              #Seleccionar contenido que se madará, el consejo o el dato del corte de energía
              ...
              if ($prioridad == 1) {
                ...
                $sql = "SELECT clubs_MSISDN.id_msisdn, clubs_MSISDN.msisdn,DATEDIFF(DATE(CONVERT_TZ(NOW(), 'UTC', 'America/Tegucigalpa')),".
    ...
    echo $sql . "\n";
	      ...
	      $registros = mysql_num_rows($clientes);
              ...
              $contenido_data_tmp = $contenido_data;
              ...
              while(($row_cliente = mysql_fetch_array($clientes)) && ($contenido_data))
              {
                  ...
                  $enee = enee($msisdn);
                  echo "Antes: $contenido_text";
                  
                  if ($enee == 1) {
                    
                        $messages = [
                            "Apaga los dispositivos en standby para reducir el consumo energetico. Aun no has registrado tu zona, registrala en servicios.pcon.vip",
                            "Utiliza bombillas LED de bajo consumo para alargar su vida util y gastar menos energia. Aun no has registrado tu zona, registrala en servicios.pcon.vip",
                            "Aprovecha la luz natural abriendo ventanas para evitar el uso de luces artificiales. Aun no has registrado tu zona, registrala en servicios.pcon.vip",
                            "Manten limpio y bien ajustado el filtro del aire acondicionado para optimizar su eficiencia. Aun no has registrado tu zona, registrala en servicios.pcon.vip"
                        ];

                        $randomIndex = array_rand($messages);
                        $randomMessage = $messages[$randomIndex];
                        $contenido_data['content'] = $randomMessage;
                        $contenido_text = $contenido_data['content'];
			            $contenido_data['id_detalle_service'] = 123456;

                  } else {
                        $contenido_data['content'] = $contenido_data_tmp['content'];
                        $contenido_text = $contenido_data_tmp['content'];
                        $contenido_data['id_detalle_service'] = $contenido_data_tmp['id_detalle_service'];
                  }

		          echo "$msisdn - $enee - $contenido_text\n";
                  ...
                  else
                  {
                      ...
                      else if ($row_servicio['type_send'] == 'Edge')
                      {
                          ...
                          #echo "$msisdn,".$row_servicio['sc_cobro'].",$contenido_text";
                      }
