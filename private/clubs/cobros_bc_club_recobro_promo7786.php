...
/*
    ...
    cobros_bc_club_recobro_trabajo1185.php

    00 07 * * wed,sat php -q /opt/lampp/htdocs/private/clubs/cobros_bc_club_recobro_trabajo1185.php id_carrier=1 prioridad=1 >> /opt/lampp/htdocs/private/clubs/cobros_bc_club_recobro_trabajo1185.log
*/
set_time_limit(0);
...
function dias_primera_suscripcion($msisdn) {

    $con = mysqli_connect('rds-compartido.pconnection.net','admin' ,'8sC3rq2iQqEztsj1' ,'primeras_altas_clubs');

    $query = "SELECT DATEDIFF(CONVERT_TZ(NOW(), 'UTC', 'America/Tegucigalpa'), date_time) AS diferencia FROM clubs_MSISDN WHERE msisdn = '$msisdn'";
    ...
    echo $sql . "\n";
	      ...
	      $registros = mysql_num_rows($clientes);
              ...
              while(($row_cliente = mysql_fetch_array($clientes)) && ($contenido_data))
              {
                  ...
                  //echo "Dias desde que se activo: " . dias_primera_suscripcion($msisdn) . "\n";

                  if (dias_primera_suscripcion($msisdn) < $row_servicio['free_days'] )
                  {
                      //Gratis
                      $id_delivery = $response_free->enviar($msisdn,$row_servicio['sc_cobro1'],$contenido_text,true);

                        echo "FREE: $msisdn,".$row_servicio['sc_cobro1'].",$contenido_text,true\n";
                  ...
                  else
                  {
                      ...
                      else if ($row_servicio['type_send'] == 'Edge')
                      {
                          $id_delivery = $response_paid->enviar($msisdn,$row_servicio['sc_cobro'],str_replace('<URL_RULETA_URL_RULETA_>', 'http://pcon.vip/'.$id_msisdn, $contenido_text),true);
                            echo "PAY: $msisdn,".$row_servicio['sc_cobro'].",str_replace('<URL_RULETA_URL_RULETA_>', 'http://pcon.vip/'.$id_msisdn, $contenido_text),true\n";
                      }
