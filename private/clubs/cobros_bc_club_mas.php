...
if($conn)
{
      if(mysql_selectdb($namedb,$conn))
      {
        $sql = "SELECT clubs_SERVICE.id_club,clubs_SERVICE.name_club,clubs_SERVICE.id_carrier,clubs_SERVICE.route,".
          ...
          " WHERE clubs_CARRIER.id_carrier = $id_carrier and clubs_COBRO_AUTO.hour_session = '8' and clubs_SERVICE.`status` = 'enable' and clubs_COBRO_AUTO.last_exe <= '$fecha' and clubs_COBRO_AUTO.$str_day_field='1' and clubs_COBRO_AUTO.status = 'enable' ORDER BY clubs_COBRO_AUTO.id_clubs DESC";
  ...
  echo $sql . "\n";
    ...
    echo $sql . "\n";
              ...
              //" as dias_alta FROM clubs_MSISDN WHERE clubs_MSISDN.id_clubs = $id_club  AND clubs_MSISDN.state = 'enable' AND clubs_MSISDN.msisdn NOT IN (SELECT DISTINCT(clubs_ENVIOS_DIARIOS.msisdn) FROM clubs_ENVIOS_DIARIOS WHERE id_clubs = $id_club) GROUP BY msisdn ORDER BY clubs_MSISDN.points DESC";
    
	$sql = "SELECT clubs_MSISDN.id_msisdn, clubs_MSISDN.msisdn,DATEDIFF(DATE(CONVERT_TZ(curdate(), 'UTC', 'America/Tegucigalpa')),".
              "case clubs_MSISDN.fecha_baja when 0 then clubs_MSISDN.fecha_alta else clubs_MSISDN.fecha_baja end)".
              " as dias_alta FROM clubs_MSISDN WHERE clubs_MSISDN.id_clubs = $id_club  AND clubs_MSISDN.state = 'enable' GROUP BY msisdn ORDER BY clubs_MSISDN.points DESC";	
