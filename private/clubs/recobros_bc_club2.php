...
/*
    Modulo de recobros, obtiene como parametros
    el id_carrier
*/
set_time_limit(0);
...
if($conn)
{
      if(mysql_selectdb($namedb,$conn))
      {
        $sql = "SELECT clubs_SERVICE.id_club,clubs_SERVICE.name_club,clubs_SERVICE.id_carrier,clubs_SERVICE.route,".
          ...
          "clubs_SERVICE.enable_points,clubs_SERVICE.free_days,clubs_SERVICE.days_to_disable,clubs_SERVICE.cobros_diarios,clubs_RECOBRO_AUTO.id_broad_conf,clubs_SERVICE.`status`,clubs_SERVICE.sufijo_text" . 
          " FROM clubs_CARRIER INNER JOIN clubs_SERVICE ON clubs_CARRIER.id_carrier = clubs_SERVICE.id_carrier INNER JOIN clubs_RECOBRO_AUTO ON clubs_SERVICE.id_club = clubs_RECOBRO_AUTO.id_clubs".
          " WHERE clubs_CARRIER.id_carrier = $id_carrier and clubs_RECOBRO_AUTO.hour_session = '$hora' and clubs_SERVICE.`status` = 'enable' and clubs_RECOBRO_AUTO.last_exe < '$fecha' and clubs_RECOBRO_AUTO.$str_day_field='1' and clubs_RECOBRO_AUTO.status = 'enable' ORDER BY clubs_SERVICE.id_club DESC";
          ...
          while($row_servicio = mysql_fetch_array($response))
          {
              ...
              $sql = "update clubs_RECOBRO_AUTO set last_exe = DATE(CONVERT_TZ(curdate(), 'UTC', 'America/Tegucigalpa')) where id_broad_conf = $id_conf";
              mysql_query($sql,$conn);
              
              $id_club = $row_servicio['id_club'];
              $cobros_diarios = $row_servicio['cobros_diarios'];
              ...
              //seleccionamos los clientes activos del club cuyo eventos cobrados sean menores a los definidos diarios
              $sql ="select A.id_msisdn,B.msisdn, B.dias_alta from". 
                ...
                " INNER JOIN". 
                " (select clubs_MSISDN.points,clubs_MSISDN.id_msisdn,clubs_MSISDN.msisdn,DATEDIFF(DATE(CONVERT_TZ(curdate(), 'UTC', 'America/Tegucigalpa')),case clubs_MSISDN.fecha_baja when 0 then clubs_MSISDN.fecha_alta else clubs_MSISDN.fecha_baja end) as dias_alta from clubs_MSISDN". 
                " where clubs_MSISDN.state = 'enable' and clubs_MSISDN.id_clubs = $id_club) as B". 
                " on A.id_msisdn = B.id_msisdn )".
                " where A.eventos_cobrados < $cobros_diarios ORDER BY B.points DESC";
