<?php
	...
	function enviarc($origen, $destino, $mensaje,$id,$comunidad){
		...
		if ($status_billing['status_billing'] == "INSUFFICIENT_FUNDS"){
			//enviar_reintento($origen, $destino, $mensaje,$comunidad);
			if (date("H") >= '14' and date("H") < '17') {
				$mensaje = "Enskripsyon 'LIFE CLUB' la paka renouvle paske w pa gen ase lajan sou kont ou. Tanpri rechaje kont ou pou itilize l. Voye STOP bay 325 pou kanpe enskripsyon an"; //cambio 22022021
	...
	function actualizacion_cobro($msisdn,$cobro){
		$query5  = "update comunidadesdigicelht.suscritos_comunidades set ultimo_cobro='$cobro',fecha_ultimo_cobro=date(CONVERT_TZ(NOW(), 'UTC', 'America/Tegucigalpa')),numero_cobros = numero_cobros + 1,cobro_acumulado=cobro_acumulado + '$cobro' where numero = '$msisdn'";
