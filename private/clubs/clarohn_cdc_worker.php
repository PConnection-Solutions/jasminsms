 ...
 $callback = function($msg) use ($conn)
{
      ...
      */
      ...
      switch($errorCode)
      {
          ...
          case 4002://Suscriptor ya suscrito al paquete OK
            ...
            if($from == '7789' && $red == 'CLARO_HN' && $club_name == 'GANA' && $to != '50431982757')
        {
            $texto2 = "PREMIO";
            $url = "http://localhost/comunout/desafio/incripcion_juegahn.php?origen=".$to."&mensaje=".$texto2."&destino=".$from."&ruta=".$red."&transid=".$transId;
        ...
        }elseif($from == '7789' && $red == 'CLARO_HN' && $club_name == 'GANA' && $to == '50431982757'){
	    $texto2 = "PREMIO";
            $url = "http://localhost/comunout/desafio/incripcion_juegahn.php?origen=".$to."&mensaje=".$texto2."&destino=".$from."&ruta=".$red."&transid=".$transId;
	        }elseif($from == '7789' && $red == 'CLARO_HN' && ($club_name == 'CRISTIANOS' || $club_name == 'SOLTEROS')){

            $url2 = "http://localhost/cristianovip/wscritianos.php?origen=".$to."&mensaje=".$texto2."&destino=".$from;
            error_log("http://localhost/cristianovip/wscritianos.php?origen=".$to."&mensaje=".$texto2."&destino=".$from."\n",3,"/opt/lampp/htdocs/crissol.log");
              $ch=curl_init();
          ...
          //case 224://Usuario en Black List
            ...
            if($from == '7789' && $red == 'CLARO_HN' && $club_name == 'GANA')
            {
              $texto2 = "BAJA";
              $url = "http://localhost/comunout/desafio/incripcion_juegahn.php?origen=".$to."&mensaje=".$texto2."&destino=".$from."&ruta=".$red."&transid=".$transId;
              //$texto2 = str_replace('CDC_submitSMS','',$text);
          ...
          case 299:
            ...
            if($from == '7789' && $red == 'CLARO_HN' && $club_name == 'GANA' && $price > 0){
              //$str = "http://localhost/comunout/desafio/incripcion_juegahn.php?origen=$to&destino=$from&mensaje=premio&flag=1&transid=$transId";
              $str = "http://localhost/comunout/desafio/incripcion_juegahn.php?origen=".$to."&mensaje=".$texto2."&destino=".$from."&ruta=".$red."&transid=".$transId."&flag=8";
              error_log("Worker: http://localhost/comunout/desafio/incripcion_juegahn.php?origen=".$to."&mensaje=".$texto2."&destino=".$from."&ruta=".$red."&transid=".$transId."&flag=1\n",3,"/opt/lampp/htdocs/raspa.log");
              $ch=curl_init();
	         ...
	         /*if ($from == '7789' && $red == 'CLARO_HN'){
              //$str = "http://localhost/comunout/desafio/incripcion_juegahn.php?origen=$to&destino=$from&mensaje=premio&flag=1&transid=$transId";
              $url = "http://localhost/ficohsa3/SMS/codigo.php?msisdn=".$to."&mensaje=".$texto2."&sc=".$from."&red=".$red."&transid=".$transId;
              $ch=curl_init();
  ...
  $channel->basic_consume('CLARO_HN', '', false, false, false, false, $callback);
