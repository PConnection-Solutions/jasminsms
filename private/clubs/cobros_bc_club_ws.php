...
if($conn)
{
      if(mysql_selectdb($namedb,$conn))
      {
        $sql = "SELECT clubs_SERVICE.id_club,clubs_SERVICE.name_club,clubs_SERVICE.id_carrier,clubs_SERVICE.route,".
          ...
          " WHERE clubs_CARRIER.id_carrier = $id_carrier and clubs_SERVICE.`status` = 'enable' and clubs_COBRO_AUTO.last_exe <= '$fecha' and clubs_COBRO_AUTO.$str_day_field='1' and clubs_COBRO_AUTO.status = 'enable' AND clubs_SERVICE.id_club = $club ORDER BY clubs_COBRO_AUTO.id_clubs DESC";
	//echo $sql . "\n";
		...
		//echo $sql . "\n";
              ...
              {*/
                  $contenido_text = $_REQUEST['mensaje'];
              /*}
              else
              {
